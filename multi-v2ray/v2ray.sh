#!/bin/bash
# Author: Jrohy https://github.com/Jrohy/multi-v2ray
# Optimizor: if1y https://github.com/if1y/ScriptCN/multi-v2ray
...
BASE_SOURCE_PATH="https://cdn.jsdelivr.net/gh/Jrohy/multi-v2ray"
...
removeV2Ray() {
    #卸载V2ray脚本
    bash <(curl -L -s https://cdn.jsdelivr.net/gh/if1y/ScriptCN/multi-v2ray/go.sh) --remove >/dev/null 2>&1
    rm -rf /etc/v2ray >/dev/null 2>&1
    rm -rf /var/log/v2ray >/dev/null 2>&1

    #卸载Xray脚本
    bash <(curl -L -s https://cdn.jsdelivr.net/gh/if1y/ScriptCN/multi-v2ray/go.sh) --remove -x >/dev/null 2>&1
...
updateProject() {
    ...
    #安装V2ray主程序
    [[ ${INSTALL_WAY} == 0 ]] && bash <(curl -L -s https://cdn.jsdelivr.net/gh/if1y/ScriptCN/multi-v2ray/go.sh)
}
