...
uninstall_old_v2ray() {
    ...
    if [[ -f /usr/local/bin/v2ray ]]; then
        ...
        if [[ $? != 0 ]]; then
            ...
            systemctl stop v2ray
            bash <(curl https://cdn.jsdelivr.net/gh/v2fly/fhs-install-v2ray/install-release.sh) --remove
            systemctl daemon-reload
        fi
    fi
}
...
install_v2ray() {
    ...
    bash <(curl https://cdn.jsdelivr.net/gh/v2fly/fhs-install-v2ray/install-release.sh)
    if [[ $? -ne 0 ]]; then
...
install_v2-ui() {
    ...
    if  [ $# == 0 ] ;then
        ...
        wget -N --no-check-certificate -O /usr/local/v2-ui-linux.tar.gz https://ghproxy.com/https://github.com/sprov065/v2-ui/releases/download/${last_version}/v2-ui-linux.tar.gz
        if [[ $? -ne 0 ]]; then
        ...
        last_version=$1
        url="https://ghproxy.com/https://github.com/sprov065/v2-ui/releases/download/${last_version}/v2-ui-linux.tar.gz"
    ...
    echo -e ""
    curl -o /usr/bin/v2-ui -Ls https://cdn.jsdelivr.net/gh/sprov065/v2-ui/v2-ui.sh
